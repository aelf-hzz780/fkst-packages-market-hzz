...
return {
  ...
  test_pr_owner_derives_representative_bounded_loop_obligations = function()
    local rows = h.core.restart_transition_table()
    local edges = owner_projection.edges(OWNER, rows, inventories)
    local representatives = expected_bounded_loop_representatives(rows, edges)
    ...
    t.eq(representatives.release, nil)
  ...
  test_pr_owner_reports_bounded_loop_class_without_frozen_witness_as_unmapped = function()
    local rows = h.core.restart_transition_table()
    local edges = owner_projection.edges(OWNER, rows, inventories)
    local removed_loop_class = "stale-lineage"
    ...
    t.eq(#result.obligations + #result.unmapped, 3)
  end,
}
