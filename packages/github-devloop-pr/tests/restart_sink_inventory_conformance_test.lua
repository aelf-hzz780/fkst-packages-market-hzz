...
local semantic_specs = {
  ...
  { "comment:pr:reconcile-blocked", "reconcile", "pipeline_fix.grant_facade_comment", "comment", "lifecycle-authoritative", "state:v1/blocked+review-reconcile|fix-reconcile:v1", "packages/github-devloop-pr/departments/reconcile/main.lua", { "restart_effect_facade.make", "\"comment:pr:reconcile-blocked\"" } },
  { "label:issue:reconcile-blocked", "reconcile", "pipeline_fix.grant_facade_label", "label", "lifecycle-authoritative", "state-label:blocked;dedup=reconcile/label", "packages/github-devloop-pr/departments/reconcile/main.lua", { "facade.emit", "\"github-proxy.github_issue_label_request\"" } },
