...
local pr_analysis = require("core.restart_analysis")

local t = h.t

local OWNER = "github-devloop-pr"
local ENTITY = { kind = "pr", repo = "owner/repo", number = 17 }
local EDGE_ID = OWNER .. "/reviewing/autonomous/review-approved"
local CAS_POLICY_ID = "cas.legacy_review_result_v1"
      ...
      successors = {
        {
          state = "merge-ready",
          kind = "autonomous",
          output_variant = "review-approved",
...
local function transition(overrides)
  local value = {
    observed_from = "reviewing",
    observed_target = "merge-ready",
    edge_kind = "autonomous",
    edge_id = EDGE_ID,
    cas_policy_id = CAS_POLICY_ID,
    cause_status = "complete",
    evidence_refs = { "cause:review-approved" },
  }
...
local function ordered_history(target)
  return {
    marker("reviewing", 1, "marker:reviewing"),
    marker(target or "merge-ready", 2, "marker:" .. (target or "merge-ready")),
  }
end
...
local function assert_anomaly(value, disposition, reason_code)
  ...
  t.eq(item.entity.kind, "pr")
  t.eq(item.entity.repo, "owner/repo")
  t.eq(item.entity.number, 17)
...
return {
  ...
  test_unsupported_explicit_edge_is_illegal_transition = function()
    local result = anomaly.analyze_observed_transition_history(
      ...
      evidence({ transition({
        ...
        edge_id = OWNER .. "/reviewing/autonomous/not-declared",
        cas_policy_id = nil,
      }) })
    )
    local item = assert_anomaly(result, "illegal-transition", "unsupported-explicit-edge")
    t.eq(item.observed_from, "reviewing")
  ...
  test_pr_owner_binder_uses_pr_rows_only = function()
    local issue_history = {
      marker("thinking", 1, "marker:thinking"),
      marker("ready", 2, "marker:ready"),
    }
    local issue_evidence = evidence({ transition({
      observed_from = "thinking",
      observed_target = "ready",
      edge_id = OWNER .. "/thinking/autonomous/consensus-reached",
      cas_policy_id = "cas.legacy_consensus_result_v1",
    }) })
    issue_evidence.owner = "github-devloop"

    local result = pr_analysis.analyze_observed_transition_history(issue_history, issue_evidence)
    local item = assert_anomaly(result, "illegal-transition", "unsupported-explicit-edge")
    t.eq(item.owner, "github-devloop-pr")
  end,
  ...
  test_analysis_does_not_mutate_inputs = function()
    ...
    input_evidence.owner = "github-devloop"
    expected_evidence = copy(input_evidence)
    pr_analysis.analyze_observed_transition_history(input_history, input_evidence)

    t.is_true(deep_equal(input_history, expected_history), "bound marker history remains byte-shaped")
    t.is_true(deep_equal(input_evidence, expected_evidence), "bound evidence remains byte-shaped")
  end,
}
