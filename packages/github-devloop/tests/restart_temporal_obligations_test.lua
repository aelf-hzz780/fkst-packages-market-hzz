...
local expected_keys = {
  ["github-devloop/issue/awaiting-pr/response-with-deadline"] = true,
  ["github-devloop/issue/blocked/response-with-deadline"] = true,
...
local function synthetic_row(kind, extra)
  local obligation = {
    obligation_id = "github-devloop/issue/synthetic/" .. kind,
    kind = kind,
    body = {},
  }
...
return {
  test_issue_temporal_index_has_exact_row_authored_key_set = function()
    ...
    for key, entry in pairs(index) do
      ...
      t.eq(entry.owner, "github-devloop")
  ...
  test_duplicate_row_authored_id_fails_closed = function()
    ...
    rows[2].temporal_obligations[1].obligation_id = rows[1].temporal_obligations[1].obligation_id
    assert_fails(function()
      temporal.derive_temporal_index("github-devloop", rows, temporal.provider_capability_matrix())
  ...
  test_orphan_and_drifting_index_entries_fail_conformance = function()
    ...
    orphaned["github-devloop/issue/orphan/response-with-deadline"] = copy_value(index[next(index)])
    assert_error_list_contains(
      temporal.index_errors("github-devloop", rows, orphaned, temporal.provider_capability_matrix()),
      "orphan index entry"
    )
    ...
    assert_error_list_contains(
      temporal.index_errors("github-devloop", rows, drifted, temporal.provider_capability_matrix()),
  ...
  test_separately_authored_provider_binding_is_rejected = function()
    ...
    assert_fails(function()
      temporal.derive_temporal_index("github-devloop", { row }, temporal.provider_capability_matrix())
  ...
  test_provider_capability_matrix_rejects_runtime_extension = function()
    ...
    assert_fails(function()
      temporal.derive_temporal_index("github-devloop", core.restart_transition_table(), matrix)
