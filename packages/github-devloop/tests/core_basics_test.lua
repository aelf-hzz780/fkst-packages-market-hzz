...
return {
  test_devloop_config_defaults_and_validation = function()
    ...
    local cfg = config.devloop_config(exec)
    ...
    responses[config.env_present_command("GITHUB_TOKEN")] = { stdout = "", exit_code = 0 }
    t.eq(config.env_present("GH_TOKEN", exec), true)
    t.eq(config.env_present("GITHUB_TOKEN", exec), false)
    t.raises(function()
