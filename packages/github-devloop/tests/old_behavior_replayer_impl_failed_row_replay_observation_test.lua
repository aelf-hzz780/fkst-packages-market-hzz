...
local OBSERVATION_PREFIX = "row-replay:github-devloop:replayer-impl-failed/"
local SITE = {
  path = "libraries/devloop/replayer.lua",
  symbol = "replay_impl_failed",
  ordinal = "row-replay/impl-failed",
}
...
local FIXTURES = json_array({
  {
    name = "retryable-failure-reraise",
    failure_reason = "codex-failed",
    failure_attempt = 1,
    expected_status = "re-raised",
    expected_reason = "retryable-implementation-failure",
  ...
  {
    name = "retry-limit-noop",
    failure_reason = "codex-failed",
    failure_attempt = 2,
    expected_status = "no-op",
    expected_reason = "implementation-failure-not-retryable",
    expected_decision = "skip-idempotent(retry-limit)",
    expected_target = "implementing",
...
local function trusted_comment(body, created_at)
  return {
    ...
    created_at = created_at or "2099-01-01T00:00:00Z",
  }
end
...
local function event_payload()
  return h.issue({
    repo = REPO,
    number = ISSUE_NUMBER,
    title = "Capture impl-failed row replay behavior",
    body = "",
    state = "OPEN",
...
local function effect_observations(raises)
  ...
  for ordinal, raised in ipairs(raises) do
    if raised.queue ~= "devloop_ready" then
      error("unclassified impl-failed row replay OLD raise: " .. tostring(raised.queue), 0)
    end
...
local function capture_runtime(fixture)
  ...
  t.eq(dispatch.state, "impl-failed", fixture.name .. ": production-derived replay state")
  t.eq(dispatch.version, VERSION, fixture.name .. ": production-derived replay version")
  t.eq(dispatch.row_from_state, "impl-failed", fixture.name .. ": production impl-failed row")
  ...
  return {
    ...
    typed_intent = {
      ...
      generation_epoch = {
        ...
        failure_attempt = fixture.failure_attempt,
      },
      lineage = {
        proposal_id = PROPOSAL_ID,
    ...
    old_inputs = {
      ...
      caller_from_states = json_array({ "impl-failed" }),
    ...
    evidence_refs = json_array({
      ...
      { kind = "runtime-row-replay-handler", ref = "libraries/devloop/replayer.lua:156-175" },
...
return {
  test_replayer_impl_failed_row_replay_old_behavior_is_real_dispatch_and_bidirectional = function()
    local fixtures = fixture_tuple_set()
    local first = capture_records()
    local second = capture_records()
    t.eq(#first, 2, "complete production-reachable impl-failed replay disposition count")
    local repeat_difference = first_difference(second, first, "old_behavior_observations[replayer-impl-failed][repeat]")
    if repeat_difference ~= nil or canonical_json(second) ~= canonical_json(first) then
      error("second OLD impl-failed row replay capture differs at " .. tostring(repeat_difference or "canonical-json"), 0)
    end
    ...
    local difference = first_difference(first, expected, "old_behavior_observations[replayer-impl-failed]")
    if difference ~= nil or canonical_json(first) ~= canonical_json(expected) then
      error("runtime-bound OLD impl-failed row replay observation differs at " .. tostring(difference or "canonical-json") .. "; runtime_records=" .. canonical_json(first), 0)
    end

    local drifted = copy_value(first)
    drifted[2].old_outcome.emitted_effects = {}
    local root = "old_behavior_observations[replayer-impl-failed][negative_control]"
    ...
    t.is_true(drift ~= nil and drift:find(root .. ".2.old_outcome.emitted_effects", 1, true) ~= nil, tostring(drift))
  end,
}
